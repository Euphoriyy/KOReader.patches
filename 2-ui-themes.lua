--[[
    This user patch adds theme functionality for the background and font color.
    It allows themes to be added, edited, and deleted.

    It requires 2-ui-font-color.lua & 2-ui-background-color.lua
--]]
...
local DEFAULT_NIGHT_THEMES = {
    ...
    -- Other
    { key = "solarized_dark", label = "Solarized Dark", bg = "#002B36", fg = "#839496", night = true },
}
...
local function pick_color_menu(touchmenu_instance, type, original_hex, callback)
    ...
    local h, s, v = hexToHSV(original_hex)
    local wheel
        ...
        { {
            ...
            callback = function()
                if ColorWheelWidget then
                    ...
                    local input_dialog
                    input_dialog = set_color_menu(touchmenu_instance,
                                        ...
                                        callback = function()
                                            ...
                                            if text ~= "" then
                                                if ColorWheelWidget then
                                                    ...
                                                    local bg_input_dialog
                                                    bg_input_dialog = set_color_menu(touchmenu_instance,
                                                        ColorType.BACKGROUND, nil,
                                                        function(bg_hex)
                                                            local fg_input_dialog
                                                            fg_input_dialog = set_color_menu(touchmenu_instance,
                                                                ColorType.FOREGROUND, nil,
                                                                function(fg_hex)
                                                                    select_mode(text, bg_hex, fg_hex)
                                                                end)

                                                            UIManager:show(fg_input_dialog)
                                                            fg_input_dialog:onShowKeyboard()
                                                        end)

                                                    UIManager:show(bg_input_dialog)
                                                    bg_input_dialog:onShowKeyboard()
                                                end
